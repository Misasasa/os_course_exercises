...
答：访问速度：寄存器>高速缓存>内存>外存。因此寄存器访问最频繁，但寄存器容量最小，放不下的变量被放在内存中，查找内存前先在高速缓存中查找所需要的内存块是否已经被加载到高速缓存中，若没有，才查找内存；内存不足时需要将部分内存块放在外存中，若在内存中查找不到，才从外存中寻找并加载到内存。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
答：因为这样能最大限度地利用存储资源，而又不拖慢系统运行。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
答：写光子映射时需要保存数量较大的光子，并将其组织为KDTree，发现内存不够。尝试优化代码，减少需要保存的数据量，节省内存。

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
答：覆盖指划分程序功能模块，不会同时执行的模块共享内存。程序开发者需要划分模块，确定共享关系。

2. 什么是交换技术？覆盖与交换有什么不同？
答：交换指整个进程地址空间的换入换出。覆盖通过内存块的切换解决物理内存不够的问题，交换通过整个进程交换解决高优先级需要的内存空间不够的问题。
...
答：时间局部性：如果程序访问了存储器的一个位置，则在一定时间内很可能再次访问该位置；空间局部性：如果程序访问了存储器的一个位置，则在一定时间内很可能访问其附近的位置；分支局部性：如果程序在条件跳转语句选择了一个分支，则下次在这条语句很可能再次选择该分支。

2. 如何提高程序执行时的局部性特征？
答：通过优化代码，尽量使代码在相近时间内访问的内存相邻。
...
答：虚拟存储只把当前指令需要的进程地址空间放在物理内存中。它实现了部分交换，使程序的内存空间大于物理内存。

2. 虚拟存储需要什么样的支持技术？
答：硬件上需要地址转换，内核上需要置换算法。

### 8.5 虚拟页式存储

 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
答：只将需要使用的页面加载到内存中，动态地将需要的页面换入内存，换出暂时不用的页面。缺页中断处理将需要使用的页面加载到内存中。

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？
答：驻留位表示是否在内存，修改位表示是否被修改，访问位表示是否被访问，保护位表示允许页面访问方式。

 2. 页式存储和虚拟页式存储的区别是什么？
答：页式存储只将内存划分成页，虚拟页式存储在此基础上实现了将不用的页存在外存中，并动态地换入换出。

### 8.6 缺页异常

1. 缺页异常的处理流程？
答：从内存中找不到需要的页面引发异常，在外存中查找需要的页面，页面换入，页表项修改，继续执行。

2. 虚拟页式存储管理中有效存储访问时间是如何计算的？
答：访存时间*（1-缺页率）+缺页异常处理时间*缺页率。
